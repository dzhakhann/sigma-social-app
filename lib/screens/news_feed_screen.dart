...
class _NewsDeckState extends State<NewsDeck>
  ...
  final GlobalKey<YtWebPlayerState> _inlineKey = GlobalKey();
  ...
  void _syncInlinePlayer() {
    ...
    if (mounted) {
      setState(() {
        ...
        // webview_flutter has no web implementation at all — on web this
        // silently renders nothing rather than throwing, which read as a
        // permanently-loading blank card. Skip straight to the "won't
        // embed" fallback (static thumbnail + play button) instead.
        _inlineFailed = kIsWeb;
      });
    }
  }
  ...
  Future<void> _playVideo(Map n) async {
    ...
    // Same reason as _syncInlinePlayer: the fullscreen player is the same
    // webview_flutter widget, which has no web implementation — opening it
    // would just be a black screen with no way out of it either.
    if (id.isEmpty || kIsWeb) {
      _openLink(link.isNotEmpty ? link : 'https://www.youtube.com/watch?v=$id');
      return;
    }
    HapticFeedback.selectionClick();
